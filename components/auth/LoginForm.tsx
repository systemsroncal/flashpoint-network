...
/**
 * Posts to /api/auth/login (Route Handler) instead of a Server Action so a
 * duplicated Origin / SITE_URL cannot 500 via Next's `new URL(origin)`.
 */
export default function LoginForm({ next, initialError, registered }: Props) {
    ...
    <div className="rounded-2xl border border-white/15 bg-white/95 p-6 text-[#111] shadow-2xl backdrop-blur sm:p-8">
      ...
      >
        ...
        <button
          type="submit"
          disabled={pending}
          className="w-full rounded-lg bg-[var(--fpn-rojo)] px-4 py-3 text-sm font-bold text-white hover:brightness-110 disabled:opacity-60"
        >
