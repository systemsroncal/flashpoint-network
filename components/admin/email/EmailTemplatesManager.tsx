      ...
      {templates.map((template) => (
        ...
        >
          <Box component="form" action={upsertEmailTemplateAction}>
            ...
            <Stack spacing={2}>
              <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
                <TextField name="name" label="Name" required fullWidth defaultValue={template.name} />
                <TextField name="slug" label="Slug" fullWidth defaultValue={template.slug} />
              </Stack>
              <TextField
                name="subject"
                label="Subject"
              ...
              <Typography variant="caption" color="textSecondary">
                Shortcodes: {"{SITE_NAME}"}, {"{CURRENT_USER_FULLNAME}"}, {"{RESET_LINK}"}, {"{CURRENT_YEAR}"}
              </Typography>
